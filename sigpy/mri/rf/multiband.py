...
def dzmbrf(pulseIn, nBands = 3, bandSep = 5, phsOpt = 'None'):
    ...
    for ii in range(0, nBands):
        b += np.exp(1j*2*np.pi/n*bandSep*np.arange(-n/2, n/2, 1)* \
            (ii - (nBands-1)/2))*np.exp(1j*phs[ii])
...
def mbPhsTab(nBands, phsType = 'phsMod'):

    # return phases to minimize peak b1 amplitude of an MB pulse

    if phsType == 'phsMod':

        if nBands < 3 or nBands > 16:
        ...
        P[11, 1:14] = np.array([3.997, 0.830, 5.712, 3.838, 0.084, 1.685, \
            5.328, 0.237, 0.506, 1.356, 4.025, 4.483, 4.084])
        P[12, 1:15] = np.array([4.126, 2.266, 0.957, 4.603, 0.815, 3.475, \
            0.977, 1.449, 1.192, 0.148, 0.939, 2.531, 3.612, 4.801])
        P[13, 1:16] = np.array([4.359, 3.510, 4.410, 1.750, 3.357, 2.061, \
            5.948, 3.000, 2.822, 0.627, 2.768, 3.875, 4.173, 4.224, 5.941])

        out = P[nBands-3, 0:nBands]

    elif phsType == 'ampMod':

        # Malik's Hermitian phases: From S J Malik, ISMRM 2015, p. 2398
        if nBands < 4 or nBands > 12:
        ...
        out = P[nBands-4, 0:nBands]

    elif phsType == 'quadMod':

        # Grissom's quadratic phases (unpublished)
        k = 3.4/nBands # quadratic phase coefficient
        out = k*(np.arange(0, nBands, 1)-(nBands-1)/2)**2;

    else:
        raise Exception('phase type ("{}") is not recognized.'.format(phsType))
