...
def correlate(input1, input2, axes=None, mode='full'):
    ...
    with device:
        ...
        input2_conj = xp.conj(util.flip(input2.reshape(ishape2_exp), axes=axes))
    ...
    def cudnn_convolve(
        ...
        if np.issubdtype(dtype, np.complexfloating):
            with device:
                ...
                yr = cudnn_convolve(xr, Wr, mode=mode, strides=strides, conv_mode=conv_mode)
                yr -= cudnn_convolve(xi, Wi, mode=mode, strides=strides, conv_mode=conv_mode)
                yi = cudnn_convolve(xr, Wi, mode=mode, strides=strides, conv_mode=conv_mode)
                yi += cudnn_convolve(xi, Wr, mode=mode, strides=strides, conv_mode=conv_mode)
        ...
        with device:
            ...
            conv_desc = cudnn.create_convolution_descriptor(pads, strides, dtype, mode=conv_mode)
...
def _get_convolve_shapes(ishape1, ishape2, axes, mode):
    ...
    for a in axes:
        if mode == 'full':
            ...
            oshape[a] = max(tshape1[a], tshape2[a]) - min(tshape1[a], tshape2[a]) + 1
