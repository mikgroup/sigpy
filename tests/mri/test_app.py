...
class TestApp(unittest.TestCase):
        ...
        for solver in ["GradientMethod", "PrimalDualHybridGradient", "ADMM"]:
            with self.subTest(solver=solver):
                img_rec = app.L1WaveletRecon(
                    ksp, mps, lamda, solver=solver, show_pbar=False
                ).run()
                npt.assert_allclose(img, img_rec, atol=1e-2, rtol=1e-2)
    ...
    def test_shepp_logan_TotalVariationRecon(self):
        img, mps, ksp = self.shepp_logan_setup()
        lamda = 0
        for solver in ["PrimalDualHybridGradient", "ADMM"]:
            with self.subTest(solver=solver):
    ...
    def test_ones_JsenseRecon(self):
        ...
        img = np.ones(img_shape, dtype=np.complex128)
        mps = sim.birdcage_maps(mps_shape)
        ksp = sp.fft(mps * img, axes=[-2, -1])

        _app = app.JsenseRecon(
            ksp, mps_ker_width=6, ksp_calib_width=6, show_pbar=False
        )
        mps_rec = _app.run()
    ...
    def test_espirit_maps_eig(self):
        ...
        mps_rec, eig_val = app.EspiritCalib(
            ksp, output_eigenvalue=True, show_pbar=False
        ).run()
